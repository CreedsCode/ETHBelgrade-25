...
function constructCommandArgs(args, cliCommands) {
    ...
    if (args && Object.values(args).find((v) => v !== undefined)) {
        if (args.forking && !cliCommands?.fork) {
            ...
            throw new errors_1.PolkadotNodePluginError('Binary path not specified.');
        }
...
async function startServer(commands, nodePath, adapterPath) {
    ...
    const currentAdapterPort = await getAvailablePort(commands.adapterCommands?.adapterPort ? commands.adapterCommands.adapterPort : constants_1.ETH_RPC_ADAPTER_START_PORT, constants_1.MAX_PORT_ATTEMPTS);
    ...
    return {
        commandArgs,
        server: new server_1.JsonRpcServer(nodePath, adapterPath),
