...
(0, config_1.subtask)(constants_1.TASK_NODE_POLKADOT_CREATE_SERVER, 'Creates a JSON-RPC server for Polkadot node')
    ...
    .setAction(async ({ nodePath, adapterPath }) => {
    const server = new server_1.JsonRpcServer(nodePath, adapterPath);
    return server;
});
...
(0, config_1.task)(task_names_1.TASK_TEST, async ({ testFiles, noCompile, parallel, bail, grep, }, { run, network, userConfig, config }, runSuper) => {
    ...
    const currentNodePort = await (0, utils_1.getAvailablePort)(userConfig.networks?.hardhat?.nodeConfig?.rpcPort
        ? userConfig.networks.hardhat.nodeConfig.rpcPort
        : constants_1.NODE_START_PORT, constants_1.MAX_PORT_ATTEMPTS);
    const currentAdapterPort = await (0, utils_1.getAvailablePort)(userConfig.networks?.hardhat?.adapterConfig?.adapterPort
        ? userConfig.networks.hardhat.adapterConfig.adapterPort
        : constants_1.ETH_RPC_ADAPTER_START_PORT, constants_1.MAX_PORT_ATTEMPTS);
    const nCommands = Object.assign({}, userConfig.networks?.hardhat?.nodeConfig, {
        port: currentNodePort,
    });
    const aCommands = Object.assign({}, userConfig.networks?.hardhat?.adapterConfig, {
    ...
    const server = new server_1.JsonRpcServer(userConfig.networks?.hardhat?.nodeConfig?.nodeBinaryPath, userConfig.networks?.hardhat?.adapterConfig?.adapterBinaryPath);
    try {
        await server.listen(commandArgs.nodeCommands, commandArgs.adapterCommands, false);
        await (0, utils_1.waitForNodeToBeReady)(currentNodePort);
        await (0, utils_1.waitForNodeToBeReady)(currentAdapterPort, true);
        await (0, utils_1.configureNetwork)(config, network, currentAdapterPort ? currentAdapterPort : currentNodePort);
        let testFailures = 0;
        try {
