...
exports.EMPTY_HARDHAT_CONFIG = void 0;
...
async function printWelcomeMessage() {
    ...
    console.log(picocolors_1.default.cyan(`⭕️ Welcome to ${constants_1.HARDHAT_POLKADOT_NAME} v${packageJson.version} ⭕️\n`));
}
...
Please delete or rename ${(0, strings_1.pluralize)(existingFiles.length, 'it', 'them')} and try again.`;
        console.log(picocolors_1.default.red(errorMsg));
        process.exit(1);
    }
    // copy the files
    for (const file of sampleProjectFiles) {
...
exports.EMPTY_HARDHAT_CONFIG = `require('@parity/hardhat-polkadot');
...
async function createProject() {
    ...
    if (isEsm && action === Action.CREATE_TYPESCRIPT_PROJECT_ACTION) {
        throw new Error(`Your project is an ESM project (you have "type": "module" set in your package.json) and you are trying to initialize a TypeScript project. This is not supported yet.`);
    }
    ...
    if (action === Action.CREATE_EMPTY_HARDHAT_CONFIG_ACTION) {
        ...
        console.log(`${picocolors_1.default.cyan(`Config file created`)}`);
    ...
    if (await canInstallRecommendedDeps()) {
        ...
        if (installedRecommendedDeps.length === recommendedDeps.length) {
            ...
            if (shouldInstall) {
                ...
                if (!installed) {
                    console.warn(picocolors_1.default.red("Failed to install the sample project's dependencies"));
                }
    ...
    console.log(`\n${picocolors_1.default.cyan('Project created')}`);
    console.log();
...
async function getDependencies(projectType) {
    const shouldInstallPeerDependencies = (await isYarnProject()) || (await isPnpmProject()) || !(await doesNpmAutoInstallPeerDependencies());
